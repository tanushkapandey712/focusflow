    ...
    <div className="flex h-full items-start gap-4">
      ...
      <div className="w-full space-y-4">
        <div className="space-y-1">
          ...
          <p className="text-sm leading-6 text-slate-600 dark:text-slate-300">Quick nudges to keep the next block clean.</p>
        </div>
        {suggestions.map((item) => (
          <div key={`${item.category}-${item.message}`} className="flex items-start gap-3 rounded-[1.35rem] bg-white/70 p-3 shadow-soft dark:bg-surface-900/45">
